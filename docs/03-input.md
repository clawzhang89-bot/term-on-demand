...
ASR 的通用模型对技术词汇（`systemctl`、`journalctl`、`/var/log`）的识别率天然偏低。手动写静态热词表能解决大部分问题，但缺点是**静态的**——你转项目、换工具栈、遇到新术语时，热词表不会自动更新。

### 思路：聊天历史驱动热词

你与 AI agent 的日常聊天历史中天然包含你正在使用的技术栈：工具名、项目名、服务器路径、部署流程中的关键词。定期从历史中提取高频技术术语，自动更新 ASR 热词表。
...
| 频次 | 权重 | 示例 |
|------|------|------|
| 出现 > 50 次 | 150 | `systemctl`, `journalctl`, `grep`, `tmux`, `nginx` |
| 出现 > 20 次 | 100 | 当前项目名（`OpenFigure`, `term-on-demand`）、常用命令 |
| 出现 > 10 次 | 50 | 偶尔使用的工具（`calibre`, `whisper`） |
| 匹配路径模式（`/.../...`） | 200 | `/var/log`, `/etc/nginx`, `~/.openclaw` |
...
| 维护工作 | 手动增删 | 全自动 |
| 对项目切换的适应 | ❌ 手动更新 | ✅ 自动跟随 |
| 对个人用词习惯的贴合 | 通用列表 | **个人定制** |
| 持续运行成本 | 无 | 接近于零（几百 token/次） |

### 落地方式

这是一个纯脚本工作，不需要改造 Voice Daemon 的主逻辑。定期（每周/每月）在本地跑一次：

1. 扫描 OpenClaw session 目录中的 JSONL 文件
2. 提取 assistant 回复中的技术词汇
3. 输出热词表 JSON
4. 部署到 Voice Daemon 的热词配置文件

热词表更新是幂等的——如果这一周没有新词出现，输出不变，不影响现有配置。

### 与场景微调的关系

场景微调（火山引擎自学习平台）需要百小时级别的音频+文本标注数据，训练成本高、迭代周期长。**对于终端命令这个场景，热词持续优化能覆盖 95% 以上的识别问题**，不需要走到微调这一步。微调只在口音极重或环境噪声不可控的场景下才有必要考虑。
